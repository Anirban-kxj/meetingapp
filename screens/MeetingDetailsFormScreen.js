...
import {View,Text, Button, TextInput, Platform, StyleSheet, Keyboard} from 'react-native';
...
const MeetingDetailsFormScreen = (props) => {
  ...
  const setDateFormat = (getDate)=>{
    ...
    if(getDate.includes("/")){ 
      ...
      console.log("Value of fullDate in else: ",getDate.substring(1,11));
  ...
  const onChange = (event, selectedDate) => {
    ...
    //console.log("Value of fullDate: ",JSON.stringify(currentDate).substring(1,13));
    //console.log("Value of fullDate: ",format(JSON.stringify(new Date(currentDate)).substring(1,11), "mm/dd/yy"));
    //setFullDate(JSON.stringify(new Date(currentDate)).substring(1,9))
    //setDate(currentDate); var getDate = JSON.stringify(getDate);
    if(mode==="date"){
    ...
    <View style={styles.container}>
      ...
      <View style={styles.saveIcon}>
        <Fontisto 
          ...
          size={28} 
          color="#327FF3" 
          onPress={props.addEventDetails.bind(this, userName, userContact, userEvent, userVisitedPlace, userComment, usercontactDuration, newDate, month, year, time, fullDate)} 
        />
      </View>
    </View>
  );
